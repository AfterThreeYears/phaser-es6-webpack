  ...
  generateBullet() {
    ...
    if (this.plane.isInjured) {
      ...
      for (let i = 40; i <= 120; i += 30) {
        let mybullet = game.add.sprite(this.plane.x, this.plane.y, 'mybullet', null, this.mybulletGroup);
  ...
  createEnemyBullet() {
    ...
    this.enemyGroup.forEachAlive((enemy) => {
      if (enemy.lastFireTime > enemy.fireGapTime) {
        const enemyBullet = this.enemyBulletGroup.getFirstExists(false, true, enemy.x, enemy.y + enemy.height / 2, 'bullet');
